...
/**
 * A cache that will handle caching files locally and maintains an in-memory cache of the same
 * files.
 */
public class LocalDirectoryCache extends SimpleForwardingCache<String, File> {
  ...
  /** Fetches files to cache directory and update in-memory cache. */
  ...
  /** Refreshes in-memory cache to match what stored in cacheDir. */
  public void refresh() {
  ...
  /** Visits the cache directory and generate map from cache key to cached file. */
  ...
  /** Removes files from cache directory and invalidate in-memory cache. */
